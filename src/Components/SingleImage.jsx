   ...
   {currentImage >= 0 && createPortal(
    <dialog id="my_modal_2" className="modal">
     <div className="modal-box bg-white">
      <button onClick={() => currentImage >= 1 && setCurrentImage(+currentImage - 1)} className="btn btn-circle btn-outline absolute left-2 top-32 min-[375px]:top-44 sm:top-64">
       <AiOutlineLeft />
      </button>
      <img src={createImageSource(Number(currentImage))} alt="image" />
      <button onClick={() => currentImage < Number(imageIndex.length - 1) && setCurrentImage(+currentImage + 1)} className="btn btn-circle btn-outline absolute right-2 top-32  min-[375px]:top-44 sm:top-64">
       <AiOutlineRight />
      </button>
